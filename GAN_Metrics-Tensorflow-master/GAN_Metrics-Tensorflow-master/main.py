...
def inception_score() :
    filenames = glob(os.path.join('C:/Users/HPC/Desktop/Fedlearning/Fed1/images/total1', '*.*'))
    ...
    fff = open('C:/Users/HPC/Desktop/LY/morematlab/IS.txt', 'a+')
    F.append(IS[0])
    F.append(IS[1])
    for num in F:
        fff.write(str(num) + '\t')
...
def frechet_inception_distance() :
    filenames = glob(os.path.join('C:/Users/HPC/Desktop/Fedlearning/Fed1/training/total', '*.*'))   #真实图片
    ...
    print("FID : ", FID / 100)
    a = FID / 100
    fff = open('C:/Users/HPC/Desktop/LY/morematlab/FID.txt', 'a+')
    F = []
    F.append(a)
    for num in F:
        fff.write(str(num) + '\t')
...
def kernel_inception_distance() :
    filenames = glob(os.path.join('C:/Users/HPC/Desktop/Fedlearning/Fed/images/total', '*.*'))
    real_images = [get_images(filename) for filename in filenames]
    real_images = np.transpose(real_images, axes=[0, 3, 1, 2])

    filenames = glob(os.path.join('C:/Users/HPC/Desktop/Fedlearning/Fed/images/AKIEC2', '*.*'))
    fake_images = [get_images(filename) for filename in filenames]
    fake_images = np.transpose(fake_images, axes=[0, 3, 1, 2])
