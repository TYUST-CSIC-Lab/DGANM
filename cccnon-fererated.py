...
class Fed_learning_SET_CNN(Fed_learning_SET):
    ...
    def round_communication(self):
        ...
        f3 = []
        f4 = []
        fff = open('C:/Users/HPC/Desktop/LY/morematlab/result.txt', 'a+')
        for r in range(self.round):
            ...
            for client in range(0, self.num_clients):
                ...
                self.Global.train_and_score(
                    self.x_train_set[i:18000+i],
                    self.y_train_set[i:18000+i],
                    local_epochs=10

                    # clients = client*8000
                    # self.Global.model.set_weights(self.current_weights)
                    # self.Global.train_and_score(
                    #     self.x_train_set[clients+0:clients+8000],
                    #     self.y_train_set[clients+0:clients+8000],
                    #     self.data.sample_weights[clients+0:clients+8000],
                    #     local_epochs=20
                )
                ...
                print('Round {0}:clientAVG_loss={1}, clientAVG_accuracy={2}'.format(r, score_client[0], score_client[1]))
                f1.append(score_client[1])
                f2.append(self.Global.noParm)
                f3.append(auc)
                f4.append(score_client[0])
        F.append(f1[-1])
        F.append(f2[-1])
        F.append(f3[-1])
        F.append(f4[-1])
...
class Mnist_CNN_shards():
    ...
    def mnist_iid_shards(self, num_clients):
        nb_classes = 7
        data1 = pd.read_csv("C:/Users/HPC/Desktop/Fedlearning/Fed/label.csv", header=None)
        list1 = data1.values.tolist()
        self.y_test_set = list1[0:2000]
        data2 = pd.read_csv("C:/Users/HPC/Desktop/Fedlearning/Fed/label3.csv", header=None)
        list2 = data2.values.tolist()
        y_train_set = list2[0:32000]
        self.x_test_set = np.load('C:/Users/HPC/Desktop/Fedlearning/Fed/test.npy')

        x_train_set = np.load('C:/Users/HPC/Desktop/Fedlearning/Fed/train2.npy')
